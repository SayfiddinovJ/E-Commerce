...
class UsersScreen extends StatefulWidget {
  const UsersScreen(
      {super.key, required this.userRepo, required this.loginRepo});
...
class _UsersScreenState extends State<UsersScreen> {
  ...
  _fletchUsers() async {
    ...
    debugPrint('USERS FETCH FINISHED ${users.length}');
                  ...
                  builder: (context) {
                    return AlertDialog(
                      ...
                      actions: [
                        ...
                        TextButton(
                          onPressed: () {
                            if (StorageRepository.deleteString('token') !=
                                null) {
                              Navigator.pop(context);
          ...
          : ListView(
              children: [
                ...List.generate(users.length, (index) {
                  debugPrint(users[index].name.firstname.capitalize());
                  return ListTile(
                    onTap: () {
                      Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) =>
                                  UserDetailScreen(userModel: users[index])));
                    },
                    leading: const Icon(
                    ...
                    title: Text(
                        '${users[index].name.firstname.capitalize()} ${users[index].name.lastname.capitalize()}'),
